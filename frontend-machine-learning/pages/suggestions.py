...
def get_and_show_recommendations(song_name: str, artist_name: str, max_recommendations: int):
    """
    Obtener y mostrar recomendaciones de canciones
    ...
    """
    with st.spinner("🔍 Buscando canciones similares con IA..."):
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                base_song = data["base_song"]
                recommendations = data["recommendations"]
                
                if recommendations:
                    show_recommendations_results(base_song, recommendations)
                else:
                    st.warning("No se encontraron recomendaciones para esta canción.")
            ...
            st.error("🚫 **No se pudo conectar con el sistema de recomendaciones.**")
            st.info("Verifica que la API esté funcionando en http://localhost:8000")
            show_fallback_suggestions()
        
        except requests.exceptions.Timeout:
            st.error("⏱️ **Tiempo de espera agotado.**")
            st.info("El sistema está tardando más de lo normal. Intenta nuevamente.")
...
def show_recommendations_results(base_song: Dict[str, Any], recommendations: List[Dict[str, Any]]):
    ...
    st.success(f"✅ Canción encontrada: **{base_song['song']}** por **{base_song['artist']}**")
...
def create_recommendation_card_html(recommendation: Dict[str, Any]) -> str:
    ...
    if (spotify_info and 
        ...
        fallback_url = get_fallback_image_url(recommendation["song"], recommendation["artist"])
    ...
    song_title = recommendation["song"].replace('"', '&quot;').replace("'", "&#39;")
