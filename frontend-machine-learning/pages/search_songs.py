...
from utils.api_client import analyze_words_safe, format_confidence
...
def create_recommendation_card(recommendation: Dict[str, Any], index: int) -> str:
    ...
    # Badge de explícito/limpio
    if recommendation.get('explicit'):
        badge_html = '<div class="explicit-badge-enhanced">🔥 EXPLÍCITA</div>'
    else:
        badge_html = '<div class="clean-badge-enhanced">✅ LIMPIA</div>'
    ...
    if analysis:
        ...
        if explicit_words:
            ...
            for i in range(0, len(explicit_words), cols_per_row):
                ...
                for j, word_info in enumerate(explicit_words[i:i+cols_per_row]):
                    with cols[j]:
                        score = word_info['explicit_score']
                        st.markdown(
                            f'<span class="explicit-word">{word_info["word"]}</span> '
        ...
        st.error("Error al analizar la canción. Verifica que la API esté funcionando.")
...
def show_pagination(current_page: int, total_pages: int):
    ...
    with col1:
        if st.button("⏮️ Primera", disabled=current_page == 1):
            st.session_state.search_page = 1
            st.session_state.selected_song_index = None  # Limpiar selección
            st.rerun()
    ...
    with col4:
        if st.button("➡️ Siguiente", disabled=current_page == total_pages):
            st.session_state.search_page = current_page + 1
            st.session_state.selected_song_index = None  # Limpiar selección
            st.rerun()
    
    with col5:
        if st.button("⏭️ Última", disabled=current_page == total_pages):
...
def show_popular_songs():
    ...
    if data_manager.df is not None and not data_manager.df.empty:
        total_songs = len(data_manager.df)
        explicit_songs = data_manager.df['is_explicit'].sum() if 'is_explicit' in data_manager.df.columns else 0
        ...
        with suggestion_cols[0]:
            if st.button("🎸 Rock/Metal", use_container_width=True):
                st.session_state.search_title = ""
                st.session_state.search_artist = ""
